...
<xsl:variable name="current-href">
   <xsl:choose>
      ...
      <xsl:otherwise>
         <xsl:text />../<xsl:value-of select="$module" />/<xsl:value-of select="$etype" />s/<xsl:value-of select="$ext" />/<xsl:text />
...
<xsl:template match="/">
   <xsl:if test="(/function or /element) and $version != 1 and not(contains($version, '.'))">
      <exsl:document href="../{$module}/{$etype}/{$ext}/{$module}.{$ext}.{$version - 1}.xml">
         <xsl:copy-of select="$current" />
      </exsl:document>
   </xsl:if>
   <exsl:document href="../{$module}/{$etype}/{$ext}/{$module}.{$ext}.xml">
      <xsl:apply-templates />
   </exsl:document>
</xsl:template>
